...
function GlowCardGrid({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const onPointerMove = (e: PointerEvent) => {
      ...
      ref.current.querySelectorAll<HTMLElement>("[data-slot='glow-card']").forEach((card) => {
        const r = card.getBoundingClientRect();
        card.style.setProperty("--pointer-x", ((e.clientX - (r.left + r.width / 2)) / (r.width / 2)).toFixed(3));
        card.style.setProperty("--pointer-y", ((e.clientY - (r.top + r.height / 2)) / (r.height / 2)).toFixed(3));
        const pointerLeft = Math.max(0, Math.min(100, ((e.clientX - r.left) / r.width) * 100));
        const pointerTop = Math.max(0, Math.min(100, ((e.clientY - r.top) / r.height) * 100));
        card.style.setProperty("--pointer-left", `${pointerLeft.toFixed(2)}%`);
        card.style.setProperty("--pointer-top", `${pointerTop.toFixed(2)}%`);
      });
    };
...
function GlowCard({
  ...
  const glowMaskStyle = glowSrc
    ? ({
        maskImage: "radial-gradient(circle 180px at var(--pointer-left, 50%) var(--pointer-top, 50%), #000 0%, #000 46%, transparent 78%)",
        WebkitMaskImage: "radial-gradient(circle 180px at var(--pointer-left, 50%) var(--pointer-top, 50%), #000 0%, #000 46%, transparent 78%)",
      } as React.CSSProperties)
    : undefined;
  ...
  return (
    <div
      data-slot="glow-card"
      className={`group @container-size relative overflow-hidden rounded-(--card-radius) bg-[color:var(--foreground)]/[0.02] ring-1 ring-[color:var(--card-border)] ${className}`}
    >
      <div className="flex size-full overflow-hidden rounded-(--card-radius) [clip-path:inset(0_round_var(--card-radius))]">
        {glowSrc && (
          <div
            className={[
              "pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-[var(--motion-panel)] ease-[var(--ease-standard)]",
          ...
          >
            <div
              className={[
                ...
                "translate-x-[calc(var(--pointer-x,-10)*32cqi)] translate-y-[calc(var(--pointer-y,-10)*32cqh)] translate-z-0 scale-(--card-icon-scale)",
      ...
      {glowSrc && (
        <div
          className={[
            ...
            "opacity-0 transition-opacity duration-[var(--motion-panel)] ease-[var(--ease-standard)]",
            "group-hover:opacity-100 group-focus-within:opacity-100",
          ].join(" ")}
          style={glowMaskStyle}
        >
          <div
            className={[
              "absolute inset-0 rounded-(--card-radius)",
